...
if __name__ == "__main__":
    ...
    for split in splits:
        os.makedirs(f'{DATAOUT}/descr/{split}', exist_ok=True)
        ...
        for scene_name in split_scenes:
            ...
            description = np.array([scene['description']])
            ...
            for data in scene_data_orig:
                # sample_data = NuScenes.get_sample_data(*data.split('_'))
                ...
                if sample['token'] in all_samples:
                    ...
                    for ann_token in sample['anns']:
                        ...
                        if attribute is None:
                            data[3] = "None"
                        elif "moving" in attribute:
                            data[3] = "Moving"
                        elif "stopped" in attribute:
                            data[3] = "Stopped"
                        elif "parked" in attribute:
                            data[3] = "Parked"
                        elif "with_rider" in attribute:
                            data[3] = "Withrider"
                        elif "without_rider" in attribute:
                            data[3] = "Withoutrider"
                        elif "sitting_lying_down" in attribute:
                            data[3] = "Sitting"
                        else:
                            data[3] = "Standing"
            ...
            total_pred += pred_num
            np.savetxt(f'{DATAOUT}/descr/{split}/{scene_name}.txt', description, fmt='%s')
