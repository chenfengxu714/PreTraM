...
def plot(mask):
    ...
    import numpy as np
    print(mask.shape)
    mask = mask.view(16, 8, 128, 128)
    mask = mask[15][0].cpu().detach().numpy()
    ones = np.ones(mask.shape)*255
    print(type(mask))
    cv2.imwrite('./mask.jpg', mask*ones)
...
def agent_aware_attention(query: Tensor,
    ...
    # print(embed_dim, embed_dim_to_check)
    ...
    if agent_aware_mask is not None:
        assert agent_aware_mask.dtype == torch.float32 or agent_aware_mask.dtype == torch.float64 or \
            agent_aware_mask.dtype == torch.float16 or agent_aware_mask.dtype == torch.uint8 or agent_aware_mask.dtype == torch.bool, \
            'Only float, byte, and bool types are supported for attn_mask, not {}'.format(agent_aware_mask.dtype)
        if agent_aware_mask.dtype == torch.uint8:
            warnings.warn("Byte tensor for attn_mask in nn.MultiheadAttention is deprecated. Use bool tensor instead.")
            agent_aware_mask = agent_aware_mask.to(torch.bool)

        if agent_aware_mask.dim() == 2:
            agent_aware_mask = agent_aware_mask.unsqueeze(0)
            if list(agent_aware_mask.size()) != [1, query.size(0), key.size(0)]:
                raise RuntimeError('The size of the 2D attn_mask is not correct.')
        elif agent_aware_mask.dim() == 3:
            if list(agent_aware_mask.size()) == [bsz, query.size(0), key.size(0)]: 
                agent_aware_mask = agent_aware_mask.repeat_interleave(num_heads, dim=0)
            if list(agent_aware_mask.size()) != [bsz * num_heads, query.size(0), key.size(0)]:
                raise RuntimeError('The size of the 3D attn_mask is not correct.')
        else:
            raise RuntimeError("attn_mask's dimension {} is not supported".format(agent_aware_mask.dim()))
    ...
    if in_proj_weight_self is not None:
        ...
        if agent_aware_mask is None:
            attn_weight_self_mask = torch.eye(num_agent).to(q.device)
            attn_weight_self_mask = attn_weight_self_mask.repeat([attn_output_weights.shape[1] // num_agent, \
                    attn_output_weights.shape[2] // num_agent]).unsqueeze(0)
        else:
            attn_weight_self_mask = agent_aware_mask
        ...
        # TODO: testing!
        # this should be equivalent to the drawing, i.e.
        # attn_output_weights_inter * (1 - attn_weight_self_mask) * attn_mask.bool() + attn_output_weights_self * attn_weight_self_mask * attn_mask.bool()
        if attn_mask is not None:
            if attn_mask.dtype == torch.bool:
                attn_output_weights.masked_fill_(attn_mask, -1e5)
               # plot(attn_mask)
            else:
...
class AgentAwareAttention(Module):
    ...
    def forward(self, query, key, value, agent_aware_mask=None, key_padding_mask=None,
...
class AgentFormerEncoderLayer(Module):
    ...
    def __init__(self, cfg, activation="relu"):
        super().__init__()
        self.cfg = cfg
        d_model = cfg.tf_model_dim
        nhead = cfg.tf_nhead
        dim_feedforward = cfg.get('tf_ff_dim', 2048)
        dropout = cfg.get('tf_dropout', 0.1)
    ...
    def forward(self, src: Tensor, agent_aware_mask=None, src_mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None,\
                 num_agent=1, need_weights=True) -> Tensor:
        ...
        """
        src2, attn_weights = self.self_attn(src, src, src, agent_aware_mask=agent_aware_mask, attn_mask=src_mask,
                            key_padding_mask=src_key_padding_mask, num_agent=num_agent, need_weights=need_weights)
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
        src = self.norm2(src)
        return src, attn_weights # attn_weights is none if need weights is false
...
class AgentFormerDecoderLayer(Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, self_agent_aware_mask=None, cross_agent_aware_mask=None,\
        ...
        Shape:
            see the docs in Transformer class.
        """
        tgt2, self_attn_weights = self.self_attn(tgt, tgt, tgt, agent_aware_mask=self_agent_aware_mask, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask, num_agent=num_agent, need_weights=need_weights)
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)
        tgt2, cross_attn_weights = self.multihead_attn(tgt, memory, memory, agent_aware_mask=cross_agent_aware_mask, attn_mask=memory_mask,
...
class AgentFormerEncoder(Module):
    ...
    def __init__(self, cfg, nlayer, norm=None):
        super().__init__()
        # self.layers = _get_clones(encoder_layer, num_layers)
        self.num_layers = nlayer
        self.layers = nn.ModuleList([AgentFormerEncoderLayer(cfg) for i in range(self.num_layers)])
        self.norm = norm

    def forward(self, src: Tensor, agent_aware_mask=None, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, \
             num_agent=1, need_weights=False) -> Tensor:
        ...
        """
        output = src
        attn_weights = [None for _ in range(self.num_layers)]
        for i, mod in enumerate(self.layers):
            output, attn_weights[i] = mod(output, src_mask=mask, agent_aware_mask=agent_aware_mask,\
             src_key_padding_mask=src_key_padding_mask, num_agent=num_agent)
...
class AgentFormerDecoder(Module):
    ...
    def __init__(self, cfg, nlayer, norm=None):
        super().__init__()
        self.num_layers = nlayer
        self.layers = nn.ModuleList([AgentFormerDecoderLayer(cfg) for i in range(self.num_layers)])
        self.norm = norm
    ...
    def forward(self, tgt: Tensor, memory: Tensor, self_agent_aware_mask=None, cross_agent_aware_mask=None,
        ...
        for i, mod in enumerate(self.layers):
            output, self_attn_weights[i], cross_attn_weights[i] = mod(output, memory, tgt_mask=tgt_mask,
                         self_agent_aware_mask=self_agent_aware_mask, 
                         cross_agent_aware_mask=cross_agent_aware_mask,
