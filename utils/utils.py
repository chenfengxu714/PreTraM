...
def create_summary_writer(log_dir, phase='train'):
    root_output_dir = Path(log_dir)
    # set up logger
    if not root_output_dir.exists():
        print('=> creating {}'.format(root_output_dir))
    ...
    time_str = time.strftime('%Y-%m-%d-%H-%M')
    logdir = '{}_{}'.format(time_str, phase)
    final_log_dir = os.path.join(root_output_dir, logdir)
    writer = tensorboardX.SummaryWriter(final_log_dir)
    return writer
