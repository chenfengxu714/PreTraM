...
class Config:
    ...
    def __init__(self, cfg_id, tmp=False, create_dirs=False):
        ...
        cfg_dir = osp.join(osp.abspath(osp.dirname(osp.dirname(__file__))), 'cfg')
        for r,d,f in os.walk(cfg_dir):
            for file in f:
                if file == '%s.yml' % cfg_id:
                    cfg_file = osp.join(r,file)
                    break
        try:
            self.yml_dict = EasyDict(yaml.safe_load(open(cfg_file, 'r')))
        except:
            print('{} not found'.format(cfg_id))
